...
const Node* isSublist(const Node* pattern, const Node* target) {
    while(target) {
        if(pattern->data == target->data) {
            const Node *pattern_cur = pattern, *target_cur = target;
            while(pattern_cur && target_cur && pattern_cur->data == target_cur->data) {
                pattern_cur = pattern_cur->next;
                target_cur = target_cur->next;
                if(pattern_cur == nullptr)
                    return target;
            }
        }
        target = target->next;
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    while(listA) {
        const Node *listB_cur = listB;
        while(listB_cur) {
            if(listA == listB_cur)
                return listA;
            listB_cur = listB_cur->next;
        }
        listA = listA->next;
    }
    return nullptr;
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB) {
    unordered_set<const Node*> suffix_head;
    while(listA) {
        suffix_head.insert(listA);
        listA = listA->next;
    }
    while(listB) {
        if(suffix_head.find(listB) != suffix_head.end())
            return listB;
        listB = listB->next;
    }
    return nullptr;
